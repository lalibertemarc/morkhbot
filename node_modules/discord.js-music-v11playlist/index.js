	...
	client.on('message', msg => {
		...
		if (message.toLowerCase().startsWith(PREFIX.toLowerCase())) {
			...
			switch (command) {
				...
				case 'playlistplay':
                    return playlistPlay(msg, suffix);
			}
			if (CLEAR_INVOKER) {
				msg.delete();
			}
...
ytapi.setup('AIzaSyBxS0xbJh7O6u434FESWvBxBHiky25TqoI');

function playlistPlay(msg, suffix) {
        if (msg.member.voiceChannel === undefined) return msg.channel.sendMessage(wrap('You\'re not in a voice channel.'));
        if (!suffix) return msg.channel.sendMessage(wrap('No video specified!'));
        const queue = getQueue(msg.guild.id);
        msg.channel.sendMessage(wrap('Searching...')).then(response => {
                var id = suffix.match('/\?list=(..................................)');
                if(id === null) {
                        var video = suffix;
                        message.reply(`\`Playlist ID = ${video}\``);
                } else {
                        var video = id[1];
                        message.reply(`\`Playlist ID = ${video}\``);
                }

                ytapi.playlistFunctions.getVideosForPlaylist(video).then((data) => {
                        var songs = data.map((video) => (video[0].title));
                        response.edit(`\`Queued: ${data[0].title}\``).then(() => {
                                queue.push(songs);
                                if(queue.length === 1) executeQueue(msg, queue);
	...
	function volume(msg, suffix) {
		...
		if (!suffix) return msg.channel.sendMessage(wrap(`The current volume is: ${dispatcher.volume * 100}`));
		
		if (isNaN(suffix)) return msg.channel.sendMessage(wrap('Volume must be a number!'));
