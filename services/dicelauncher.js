...
function launcher(dice, author) {
    ...
    if (number <= 0) {
        return "You rolled  a 0, congratz!";
    }
    if (face <= 0) {
        return "Please use dices with at least 1 face";
    }
    ...
    if (Number.isNaN(somme)) {
        return `${author} used a wrong command, maybe use !calc instead.`;
    }
    if (number == 1 && face == 20 && somme == 20) {
        return `${author} rolled a 20! Critical hit!`;
    }
    if (number == 1 && face == 20 && somme == 1) {
        return `${author} rolled a 1! Fumble!`;
    }
    if (somme == face * number) {
        return `${author} rolled a ${somme}. Best in possible outcome.`;
    }
    if (number > 1) {
