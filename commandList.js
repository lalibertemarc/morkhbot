...
const duelRoll = new Command(
    ...
    function (message, args) {
        ...
        var commandResponse = duel.duelRoll(
            message.author.username,
            typeRoll[1]
        );
        helpers.commandResponse(message, this, commandResponse);
...
var addMe = new Command(
    ...
    async function (message, args) {
        ...
        try {
            ...
            commandResponse = error.response.data;
        }
        helpers.commandResponse(message, this, commandResponse);
    }
);
        ...
        if (args.length != 2 || !helpers.isNumber(args[1]))
            ...
            try {
                ...
                if (response.status == 200) {
                    ...
                    let url = `http://${process.env.HOST}:${process.env.PORT}/${process.env.WRITE_API}/points`;
                    let insertResponse = await webResquestHelper.putAsync(url, {
                        name: user,
                        points: points,
                    });
                    commandResponse = insertResponse.data;
                }
            } catch (error) {
                console.log(error);
                commandResponse = "Unexpected Error, please retry";
...
let getMinecraftUUID = new Command(
    "!mcUUID",
    "Get player UUID",
    async function (message) {
