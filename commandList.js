...
const isPrime = new Command(
  ...
  function(message, args) {
    ...
    if (helpers.isNumber(args[0]))
      commandResponse = prime.isPrime(+args[0], [1]);
...
const gcd = new Command(
  ...
  function(message, args) {
    ...
    if (helpers.isNumber(args[0]) || helpers.isNumber(args[1]))
      commandResponse = prime.gcd(+args[0], +args[1]);
...
const endDuel = new Command("!endDuel", "Ends the duel", function(
  message,
  args
) {
    ...
    try {
      ...
      if (existsResponse.length == 0) {
        let insertResponse = await mongoService.insertOneInCollectionAsync(
          "points",
          { name: message.author.username, points: 0 }
        );
        if (insertResponse.insertedCount == 1)
          commandResponse = `${message.author.username} was added to database and has 0 point.`;
        else commandResponse = "Unexpected Error, please retry";
      } else
        commandResponse = `${existsResponse[0].name} has ${
          existsResponse[0].points
        } ${
          existsResponse[0].points == 0 || existsResponse[0].points == 1
            ? "point"
            : "points"
        }.`;
    } catch (error) {
...
var weather = new Command(
  ...
  async function(message) {
    ...
    try {
      ...
      commandResponse += `Wind : ${body.wind.speed *
        3.6} km/h to ${helpers.getCardinal(body.wind.deg)}`;
...
let getMinecraftUUID = new Command("!mcUUID", "Get player UUID", async function(
  message
) {
...
let minecraftNetherCoord = new Command(
  "!mcNetherCoords",
